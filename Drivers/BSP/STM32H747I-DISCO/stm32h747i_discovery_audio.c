    ...
    if(ret == BSP_ERROR_NONE)
    {
      ...
      else
      {
        ...
        if(ret == BSP_ERROR_NONE)
        {
          ...
          /* --- CORRECTION START: Standardize Output Configuration --- */
          mx_sai_config.AudioFrequency    = AudioInit->SampleRate;
          mx_sai_config.AudioMode         = SAI_MODEMASTER_TX; // Output is MASTER
          ...
          /* FIXED: Frame Length 64 for standard I2S (was 128) */
          ...
          /* FIXED: Use Slots 0 & 1 for Standard Stereo (was 0 & 2) */
          mx_sai_config.SlotActive        = SAI_SLOTACTIVE_0 | SAI_SLOTACTIVE_2;
          ...
          /* ... (Keep existing Callback registration code) ... */
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1U)
...
int32_t BSP_AUDIO_IN_Init(uint32_t Instance, BSP_AUDIO_Init_t* AudioInit)
{
  ...
  else
  {
    ...
    if(Instance == 0U)
    {
      ...
      /* --- CORRECTION START: Match Input Config to Output --- */
      ...
      mx_config.AudioMode         = SAI_MODESLAVE_RX; // Slave to Block A
      
      /* FIXED: Match Output Strobing (Rising Edge) */
      mx_config.ClockStrobing     = SAI_CLOCKSTROBING_RISINGEDGE; 
      mx_config.Synchro           = SAI_SYNCHRONOUS; // Sync to Block A
      ...
      /* * DELETED: The Block A re-initialization logic has been removed here.
       * It was causing the Clock Phase Mismatch. 
       * Assumption: You MUST call BSP_AUDIO_OUT_Init() before calling BSP_AUDIO_IN_Init().
       */

      /* ... (Keep existing Callback registration and Codec Init code) ... */
#if (USE_HAL_SAI_REGISTER_CALLBACKS == 1U)
    ...
    // ... (Keep existing Instance 1 / PDM code) ...
    else if(Instance == 1U)
    {
       // ... (Keep existing Instance 1 code as is) ...
       /* PLL clock is set depending by the AudioFreq (44.1khz vs 48khz groups) */
