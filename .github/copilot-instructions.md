...
## Build Commands
```bash
cmake --preset Debug              # Configure (uses Ninja)
cmake --build build/Debug         # Build both cores
...
- `INITIAL_MENU` → `AUDIO_EFFECTS` or `PITCH_DETECTION`
- Mode-specific code isolated in `*_mode.cpp` files
